...
export class AuthentificationService {
  ...
  isLogged() {
    return JSON.parse(localStorage.getItem("is_logged"));
  }

  login(email, mdp) {
    return new Promise((resolve, reject) => {
      this.login_s(email, this.md5(mdp)).then((reponse: any) => {
        console.log(reponse)
        if (reponse.result == "ok") {
          localStorage.setItem("is_logged", "true");
          //localStorage.setItem("userInfo", JSON.stringify(reponse.user));
  ...
  API_ENDPOINT = "http://40.89.167.111:8085/";

  login_s(email, mdp) {
    return this.post(this.API_ENDPOINT + 'login', {
      email: email,
      password: mdp
    });
  }
